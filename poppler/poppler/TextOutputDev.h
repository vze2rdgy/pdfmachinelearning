...
public:
  ...
  bool IsFontBold() { 
      if (font)
      {
        auto fontInfo = font[0];
        return fontInfo->isBold();
      }
      return false;
  }
...
public:
  ...
  XYWH<double> getXYWH(size_t pos = 0) ; 
...
public:
  ...
  // Alternative coalesce strings that look like parts of the same line.
  void coalescealt(GBool physLayout, double fixedPitch, GBool doHTML);
  ...
  int getTextBlocks(TextBlockMap* blockMap, double xMin, double yMin,
		     double xMax, double yMax);
...
public:
  ...
  // Get text in blocks in the page
  int getTextBlocks(TextBlockMap* blockMap, double xMin, double yMin,
		   double xMax, double yMax) ;

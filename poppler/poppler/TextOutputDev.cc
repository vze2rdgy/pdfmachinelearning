...
int TextLineFrag::cmpXYColumnPrimaryRot(const void *p1, const void *p2)
{
            ...
            frag1->line->col[frag1->start]))
    {
        ...
        switch (frag1->line->blk->page->primaryRot)
        {
        case 0: cmp = frag1->yMin - frag2->yMin;
            break;
        case 1: cmp = frag2->xMax - frag1->xMax;
            break;
        case 2: cmp = frag2->yMin - frag1->yMin;
            break;
        case 3: cmp = frag1->xMax - frag2->xMax;
            break;
        }
        return cmp < 0 ? -1 : cmp > 0 ? 1 : 0;
    }
...
void TextPage::coalesce(GBool physLayout, double fixedPitch, GBool doHTML)
{
...
#ifdef DBG_COALESCE // for debugging
    ...
    for (blk = blkList; blk; blk = blk->next)
    {
        ...
        for (line = blk->lines; line; line = line->next)
        {
            ...
            for (word0 = line->words; word0; word0 = word0->next)
            {
                printf("    word: x=%.2f..%.2f y=%.2f..%.2f base=%.2f fontSize=%.2f isbold=%s space=%d: '",
                       word0->xMin, word0->xMax, word0->yMin, word0->yMax,
                       word0->base, word0->fontSize, 
                       (word0->IsFontBold() ? "Y" : "N"),
                       word0->spaceAfter);
                for (i = 0; i < word0->len; ++i)
                {
                    fputc(word0->text[i] & 0xff, stdout);
...
void TextPage::coalescealt(GBool physLayout, double fixedPitch, GBool doHTML)
{
    UnicodeMap *uMap;
    TextPool *pool;
    TextWord *word0, *word1, *word2;
...
int TextPage::getTextBlocks(TextBlockMap* blockMap, double xMin, double yMin, double xMax, double yMax)
{
    if (!blockMap)
        return 0;
    ...
    //TODO: go through all blocks and extract lines. save lines as single string in blockMap
    UnicodeMap *uMap;
    ...
    if (rawOrder)
    {
        ...
        blockMap->insert(make_pair(0, TextLineMap{ std::make_pair(XYWHf(0), ostr.str())} ));
    ...
    typedef std::vector<TextLine*> vTextLines;
    ...
    for (i = 0; i < nBlocks; ++i)
    {
                ...
                yMin < blk->yMax && blk->yMin < yMax)
        {
            ...
            if (tlines.size() > 1)
            {
                std::sort(tlines.begin(), tlines.end(), TextLine::cmpXY);
            }
            
            TextLineMap lineMap;
            for (TextLine* tline : tlines)
            {
                
                std::ostringstream ostr;
                TextWord* word;
                for (word = tline->words; word; word = word->next)
                {
                    GooString s;
                    col += dumpFragment(word->text, word->len, uMap, &s);
                    auto sxml = s.xmlEncode();
                    //auto ss = s.xmlEncode();
                    ostr << sxml->getCString();
                    ostr << space;
                    delete sxml;
                }
                ostr << eol;
                lineMap.insert(std::make_pair(tline->getXYWH(m++), ostr.str()));
            }
            blockMap->insert(std::make_pair(n++, lineMap));
        }
    }    
    return blockMap->size();
}
                                    ...
                                    PDFRectangle *selection)
{
    ...
    { // a table
        ...
        if (currentBlock == frag.line->blk)
        { // the same block
            startLine();
        }
        else
        { // another block
            if (currentBlock->tableEnd)
                              ...
                              SelectionStyle style)
{
    ...
    XYWHf TextLine::getXYWH(size_t pos) 
    {
        auto r = XYWHf(pos);
        r.X = xMin;
        r.Y = yMin;
...
{
    text->endPage();
    text->coalescealt(physLayout, fixedPitch, doHTML);
    if (outputStream)
    {
...
int TextOutputDev::getTextBlocks(TextBlockMap* blockMap, double xMin, double yMin,
		   double xMax, double yMax)
{
    return text->getTextBlocks(blockMap, xMin, yMin, xMax, yMax);
}

...
const INIT = of('lynx-init-effect-trigger');
...
export class SettingsEffects {
  ...
  @Effect()
  retrieveSettings = this.actions$.pipe(
    ofType<ActionSettingsRetrieve>(SettingsActionTypes.RETRIEVE),
    switchMap((action: ActionSettingsRetrieve) =>

      this.settingsService.GetSetting(action.payload.locID).pipe(
        map(settings => {
          return new ActionSettingsRetrieveSuccess({ settings: settings });
        }),
        catchError(error => {
          return of(new ActionSettingsRetrieveError({ error: error }))
        })
      ))
  );
  ...
  updateTuitions = this.actions$.pipe(
    ofType<ActionSettingsUpdate>(SettingsActionTypes.UPDATE),
    switchMap((action: ActionSettingsUpdate) =>
      this.settingsService.UpdateSettings(action.payload.data, action.payload.locID).pipe(
        map(response => {
          return new ActionSettingsUpdateSuccess({ data: response });
        },
          catchError(error => {
            return of(new ActionSettingsUpdateError({ error: error }));
          })
        )
      )
    )
